...
def test_workspace_knowledge_center_reports_real_files_and_context_bundles(tmp_path):
    ...
    assert len(data["wiki"]) == 3
    assert {page["source"] for page in data["wiki"]} == {".sarathi/wiki"}
...
def test_workspace_wiki_and_context_bundle_endpoints_are_readable(tmp_path):
    ...
    assert status == 200
    assert [page["path"] for page in wiki_data["pages"]] == [
        "README.md",
        "architecture.md",
        "development.md",
    ]
    assert {page["source"] for page in wiki_data["pages"]} == {".sarathi/wiki"}

    status, page_data = assert_ok(request(app, "GET", f"/api/workspaces/{workspace_id}/wiki/README"))
    assert status == 200
    assert page_data["page"] == "README"
    assert "Workspace Wiki" in page_data["content"]
...
def test_workspace_wiki_prefers_generated_sarathi_wiki(tmp_path):
    app = create_app(tmp_path / "sarathi.db")
    task = create_completed_task(app, tmp_path)
    workspace_id = task["workspace_id"]
    legacy_wiki = tmp_path / "wiki"
    generated_wiki = tmp_path / ".sarathi" / "wiki"
    legacy_wiki.mkdir()
    generated_wiki.mkdir(parents=True, exist_ok=True)
    (legacy_wiki / "README.md").write_text("# Legacy Wiki\n", encoding="utf-8")
    (generated_wiki / "README.md").write_text(
    ...
    status, wiki_data = assert_ok(request(app, "GET", f"/api/workspaces/{workspace_id}/wiki"))
    assert status == 200
    readme = next(page for page in wiki_data["pages"] if page["path"] == "README.md")
    assert readme["source"] == ".sarathi/wiki"
    assert readme["generated"] is True

    status, page_data = assert_ok(request(app, "GET", f"/api/workspaces/{workspace_id}/wiki/README"))
    assert status == 200
    assert "Generated Wiki" in page_data["content"]
    assert page_data["path"].endswith(".sarathi/wiki/README.md")
