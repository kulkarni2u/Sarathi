"""OpenCode SDK-backed provider adapter with CLI fallback."""
...
class OpenCodeSdkProviderAdapter(ProviderAdapter):
    """Dispatch through the OpenCode SDK, with a safe CLI fallback."""
    ...
    def create_session(self, request: DispatchRequest | None = None) -> ProviderSession:
        return ProviderSession(
            session_id=f"opencode-sdk-{uuid4().hex[:12]}",
    ...
    def _run_sdk_bridge(self, payload: Mapping[str, Any], *, timeout_seconds: int) -> dict[str, Any]:
        ...
        if not helper_script.exists():
            raise FileNotFoundError(f"OpenCode SDK helper not found: {helper_script}")
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenCode SDK helper returned non-JSON output: {stderr or raw_output}"
            ) from exc
        if completed.returncode != 0 and "error" not in parsed:
            parsed = {
                "success": False,
                "artifacts": {"invocation_kind": "sdk", "sdk_family": "opencode"},
                "error": completed.stderr.strip() or f"OpenCode SDK helper exited with {completed.returncode}",
            }
    ...
    def _default_helper_script(self) -> str:
        ...
        return str(repo_root / "desktop" / "scripts" / "opencode-sdk-dispatch.mjs")
