...
Sarathi works without cloud services. Optional integrations, including NCP and
MCP clients, layer on top of the local runtime.
...
python3 -m pip install -e ".[tui,mcp,ncp,dev]"
```
...
| `ncp` | Neural Context Protocol sidecar dependency |
...
Sarathi works without NCP. Native adapters handle context compilation,
persistence, and artifact storage locally.

NCP is an optional sidecar that adds persistent cross-session memory, cross-agent
whispers, pattern-aware context, and pipeline cost tracking.

```bash
python3 -m pip install -e ".[ncp]"
...
Sarathi can auto-detect NCP when `.ncp/run.py` exists in the project root.
